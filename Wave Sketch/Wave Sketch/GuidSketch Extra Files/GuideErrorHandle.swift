...
extension GuidedSketchViewController {
    ...
    func ErrorFinder(HeaderInformation: [String] ) -> (Bool, String) {
        ...
        //MARK: - Error Check Voltage Values
        ...
        //MARK: - Error Check Entered Unit
        if HeaderInformation[1] == "Max Voltage Unit" || HeaderInformation[1] == "Volt Unit"{
            errorMessage = "No Max Voltage Unit Entered"
            return (false, errorMessage)
            
        }
        
        if HeaderInformation[3] == "Min Voltage Unit"  || HeaderInformation[3] == "Volt Unit"{
    ...
    //MARK: - Error Checking Min Voltage Unit With Values
        ...
        if minVoltage > 4000 && (HeaderInformation[3] == "mV" || HeaderInformation[3] == "- mV") {
            if HeaderInformation[3] == "mV" {
                errorMessage = "Min Voltage Out Of Range For Selected Unit (Max 4000mV)"
                return (false, errorMessage)
            } else{
                errorMessage = "Min Voltage Out Of Range For Selected Unit (Max -4000mV)"
                return (false, errorMessage)
        ...
        //MARK: - Check Min V Against Max V
        ...
        if maxVoltage != minVoltage && (maxVoltage - minVoltage < 0.01) {
            errorMessage = "Max Voltage And Min Voltage Must Be Equal or Have A Minimun 10mV Difference"
            return (false, errorMessage)
        }
        
        if guideDrawView.SquareWave == true && maxVoltage == minVoltage{
            errorMessage = "Cant Have Max Voltage = Min Voltage When Drawing A Square Wave"
            return (false, errorMessage)
        }
        if guideDrawView.TriangleWave == true && maxVoltage == minVoltage{
            errorMessage = "Cant Have Max Voltage = Min Voltage When Drawing A Triangle Wave"
            return (false, errorMessage)
        }
        if guideDrawView.SawtoothWave == true && maxVoltage == minVoltage{
            errorMessage = "Cant Have Max Voltage = Min Voltage When Drawing A Sawtooth Wave"
            return (false, errorMessage)
        }
        if guideDrawView.TrapazoidalWave == true && maxVoltage == minVoltage{
            errorMessage = "Cant Have Max Voltage = Min Voltage When Drawing A Trapazoidal Wave"
            return (false, errorMessage)
        }
        if guideDrawView.SinusoidalWave == true && maxVoltage == minVoltage{
            errorMessage = "Cant Have Max Voltage = Min Voltage When Drawing A Sinusoidal Wave"
            return (false, errorMessage)
        }
        ...
        if HeaderInformation[5] == "Frequency Unit" {
        ...
        if HeaderInformation[8] == "Output Channel" || HeaderInformation[8] == "Port" {
        ...
        if guideDrawView.DCWave == true && maxVoltage != minVoltage {
            
            errorMessage = "Max Voltage and Min Voltage Must Be Equal For DC Wave"
            return (false, errorMessage)
            
        }
        
        //MARK: Check Chose Wave
